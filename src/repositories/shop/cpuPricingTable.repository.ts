...
import { cpu_pricing_table } from '@prisma/client';
import { Injectable, Logger } from '@nestjs/common';
import { PrismaService } from '../../services/infra/prisma.service';
import { v4 as uuidV4 } from 'uuid';

@Injectable()
export class CpuPricingTableRepository extends BaseRepository<
  cpu_pricing_table,
  IPricingTable
> {
  private readonly logger = new Logger(CpuPricingTableRepository.name);

  constructor(private readonly prisma: PrismaService) {
    super();
  }

  protected _transform(entity: cpu_pricing_table): IPricingTable {
    return {
      id: entity.id,
  ...
  async findBy(query: IPricingTableQuery): Promise<IPricingTable | null> {
    try {
      if ('id' in query) {
        const pricingTable =
          await this.prisma.cpu_pricing_table.findUniqueOrThrow({
            where: {
              id: query.id,
            },
          });
      ...
      const pricingTable = await this.prisma.cpu_pricing_table.findFirstOrThrow(
        {
          where: {
  ...
  async create(dto: IPricingTableCreate): Promise<IPricingTable> {
    try {
      const pricingTable = await this.prisma.cpu_pricing_table.create({
        data: {
          id: uuidV4(),
          shop_id: dto.shopId,
          sheets: dto.sheets,
  ...
  async update(dto: IPricingTableUpdate): Promise<IPricingTable> {
    try {
      const pricingTable = await this.prisma.cpu_pricing_table.update({
        where: {
          id: dto.id,
        },
        data: {
